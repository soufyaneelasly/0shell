...
pub fn execute(args: &[String], _executor: &crate::executor::Executor) -> Result<ExecutionResult, ExecutorError> {
    // Convert ExecutorError and return result
    match run_ls(args) {
        Ok(()) => Ok(ExecutionResult::default()),
        Err(err) => Err(ExecutorError::IoError(std::io::Error::new(std::io::ErrorKind::Other, err))),
    }
}
...
fn print_long_entry(
    ...
    // Follow symlink and append suffix based on target
    if ftype == 'l' {
        if let Some(path) = full_path {
